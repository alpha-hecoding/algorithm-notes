...
- 初始化：理论上`dp[0][0~n-1]=1,dp[0~m-1][0]=1`，即到达第一行的任一位置只有一条路径，到达第一列的任一位置也只有一条路径，但一旦遇到障碍物，则障碍物位置及其以后位置都不可达，需要初始为0.
```java
class Solution {  
...
返回 _你可以获得的最大乘积_ 。
## 题解
- dp数组：`dp[i]`表示对整数i按照规则拆分得到的最大化乘积
- 递推公式：`dp[i] = max(dp[i], max(j, dp[j]) * (i-j))`，即表示将一个拆为两个数时，取其中一部分的数值与其拆分最大化乘积二者的最大值，与剩下另一部分的乘积，再取所有拆法中的最大值
- 初始化：理论上只需要初始化`dp[2]=1`，因为`dp[0]`、`dp[1]`不可能被取到
```java
class Solution {  
    public int integerBreak(int n) {  
        // dp[i] 表示正整数i拆分为k个正整数和时这些正整数乘积最大值  
        int[] dp = new int[n + 1];  
        // 递推公式：dp[i] = max(dp[i], max(j, dp[j]) * (i-j))  
        dp[2] = 1; // 可拆为1+1  
        for (int i = 3; i <= n; i++) {  
            for (int j = 1; j <= i - 1; j++) {  
...
## 题解
- dp数组：`dp[i]`表示有i个节点时不同的二叉搜索树数量
- 递推公式：`dp[i] = sum(dp[j] * dp[i-j-1]); j = 0 ~ i-1;`表示将一棵二叉树拆成左右两棵子树的组合
- 初始化：`dp[0] = 1`; `dp[1] = 1`;
```java
class Solution {  
    public int numTrees(int n) {  
        // dp[i]表示有i个节点时不同的二叉搜索树数量  
        int[] dp = new int[n + 1];  
        // 递推公式：dp[i] = sum(dp[j] * dp[i-j-1]); j = 0 ~ i-1  
        // 表示将一棵二叉树拆成左右子树的组合  
        dp[0] = 1; // 空节点  
        dp[1] = 1;  
        for (int i = 2; i <= n; i++) {  
